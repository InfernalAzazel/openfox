...
class LocalSkills(SkillLoader):
    ...
    Args:
        ...
        skip_hyphen_suffixed: When True (default), do not load skill folders whose
            directory name ends with ``-`` (e.g. ``draft-``). Applies to each
            child folder when ``path`` is a skills root, and to the folder itself
            when ``path`` points at a single skill directory.
    """
    ...
    def __init__(
        ...
        validate: bool = True,
        *,
        skip_hyphen_suffixed: bool = True,
    ):
        self.path = Path(path).resolve()
        self.validate = validate
        self.skip_hyphen_suffixed = skip_hyphen_suffixed
    ...
    def load(self) -> List[Skill]:
        ...
        if skill_md_path.exists():
            # Single skill folder
            if self.skip_hyphen_suffixed and self.path.name.endswith("-"):
                log_debug(f"Skipping hyphen-suffixed skill folder: {self.path}")
            else:
            ...
            for item in self.path.iterdir():
                if self.skip_hyphen_suffixed and item.name.endswith("-"):
                    log_debug(f"Skipping hyphen-suffixed skill folder: {item}")
                    continue
